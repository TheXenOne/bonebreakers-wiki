---
title: Gnolls of Westmarsh
type: faction
tags:
  - faction
  - military
  - monster
aliases:
  - Westmarsh gnolls
  - factions/military-and-orders/gnolls-of-westmarsh
status: active
primary_location: Westmarsh
notable_members:
  - Grazka
related_locations:
  - Westmarsh
  - West Fort
  - Spider Market
source_sessions:
  - 34
  - 37
  - 39
---

## Overview

The gnolls of Westmarsh are a recurring raiding threat across the frontier. They are less clearly centralised than the hobgoblin invasion force, but the published sessions repeatedly present them as active, bold, and organised enough to worry both locals and the party.

## Purpose or role

They appear mainly as raiders, patrols, and camp attackers rather than as a visible state-like power.

## Leadership

- Interrogated gnolls mention a matriarch named Grazka.
- Grazka's authority and exact role are not independently confirmed. Needs review.

## Notable members

- Grazka

## Associated locations

- [[locations/world/westmarsh|Westmarsh]]
- [[locations/world/agria/west-fort|West Fort]]
- [[locations/world/westmarsh/spiders-market|The Spider Market]]

## Allies and enemies

- The sources do not clearly establish the gnolls as part of the hobgoblin command structure.
- They are hostile to the [[factions/other/bonebreakers|Bonebreakers]] and threaten traffic near [[locations/world/agria/west-fort|West Fort]].

## Campaign involvement

- In [[sessions/session-034|Session 34]], gnolls ambush the party and interrogation brings up the name Grazka.
- In [[sessions/session-037|Session 37]], a gnoll attack at camp reinforces how unstable Westmarsh has become.
- Later siege-era material still treats gnoll patrols and raids as an ongoing danger.

## Current status

Active.

## Related sessions

- [[sessions/session-034|Session 34]]
- [[sessions/session-037|Session 37]]
- [[sessions/session-039|Session 39]]

## Unresolved threads

- Grazka and any deeper gnoll power structure remain unresolved.
- Their relationship, if any, to the Grey Syndicate or the hobgoblin war effort is unclear.
