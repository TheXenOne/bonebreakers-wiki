---
title: Eleftheria's Rudder
tags:
  - item
  - magic-item
  - ship
source_sessions:
  - 16
  - 17
---

## Overview

Eleftheria's Rudder is a small magical version of the [[locations/world/fos-imeras|Eleftheria]]'s rudder. The Bonebreakers currently hold it as a shared party item connected to summoning, assembling, or otherwise calling forth the Eleftheria, Ossilicus's magical ship.

## What it does

- The rudder is tied directly to the Eleftheria rather than being only a symbolic token.
- The Eleftheria can be summoned or called forth through a ritual offering linked to the rudder.
- The ship is also known to be modular and unusually easy to conceal or reassemble once under the party's control.

## Summoning requirements

To summon the Eleftheria, the party knows the ritual requires:

- a villain's rib
- a pound of gold
- something purple
- something old

These were the same riddle components used to call the ship from the lake on [[locations/world/fos-imeras|Fos Imeras]].

## Known history

- In [[sessions/session-014|Session 14]], the party is tasked with finding Ossilicus and recovering the Eleftheria.
- In [[sessions/session-016|Session 16]], Athlestan remembers the lake riddle and the Eleftheria rises when the needed offerings are given.
- In [[sessions/session-017|Session 17]], the party learns more about the ship's modular magical nature.
- The party later keeps the smaller rudder as the item connected to calling or managing the ship. Exact later handling details: Needs review.

## Related people and places

- [[npcs/fos-imeras/orsilochus|Ossilicus]]
- [[locations/world/fos-imeras|Fos Imeras]]
- [[quests/major/completed/fos-imeras-and-the-golden-asclepion|Fos Imeras and the Golden Asclepion]]

## Related sessions

- [[sessions/session-014|Session 14]]
- [[sessions/session-016|Session 16]]
- [[sessions/session-017|Session 17]]

## Open questions

- It is not yet clear whether the rudder alone is sufficient to summon the Eleftheria anywhere, or whether location and other conditions matter.
- The exact limits of the ship's modular reassembly are still unclear.
